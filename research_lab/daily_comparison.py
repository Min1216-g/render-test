...
def daily_result_message(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "기존 AI",
        f"정확도: {result.get('existing_accuracy')}%",
        f"평균 수익률: {existing_return:+.1f}" if isinstance(existing_return, (int, float)) else f"평균 수익률: {existing_return}",
        f"점수: {result.get('existing_daily_score')}",
        "",
        "Research AI",
        f"정확도: {result.get('research_accuracy')}%",
        f"평균 수익률: {research_return:+.1f}" if isinstance(research_return, (int, float)) else f"평균 수익률: {research_return}",
        f"점수: {result.get('research_daily_score')}",
        "",
        "━━━━━━━━━━━━━━",
        "",
        "🏆 승자",
        str(winner).upper(),
        "",
        f"둘 다 적중: {result.get('both_correct')}",
        f"기존만 적중: {result.get('existing_only')}",
        f"Research만 적중: {result.get('research_only')}",
        f"둘 다 실패: {result.get('both_wrong')}",
        "",
        "━━━━━━━━━━━━━━",
        "",
        "결과:",
